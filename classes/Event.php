...
class Event extends CorePage {
	...
	public function getEventList() {
		$results = [];
		$stmt = $this->ci->db->query("select e.id, r.id as res_id, h.id as host_id, h.name as host_name, r.name, et.name as type, e.property, e.current_value, e.oper, round(e.value) as value, e.start_time from res_events e, ressources r, event_types et, hosts h where e.res_id=r.id and et.id=e.event_type and e.end_time is null and h.id = e.host_id order by et.id asc,e.start_time asc");
