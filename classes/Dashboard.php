...
select et.name, et.id, count(e.id) as cnt 
 ...
 order by id");
	...
	public function getActivesEvents() {
		$ret = [];
		$s = $this->ci->db->prepare("select h.name as host_name, e.id, r.name, et.name as type, e.property, e.current_value, e.oper, e.value, e.start_time 
 ...
 where h.id = e.host_id and e.res_id=r.id and et.id=e.event_type and e.end_time is null order by et.id");
	...
	public function getHostDomains() {
		$ret = [];
		$s = $this->ci->db->prepare("select 0 as id, \"unset\" as name, count(*) as cnt 
  from h\$hosts 
...
select id, name, ifnull(h.cnt, 0) as cnt 
 ...
 order by id");
	...
	public function getMonitoringItems() {
		$ret = [];
		$s = $this->ci->db->prepare("select et.name, et.id, ifnull(e.cnt,0) as cnt 
 from c\$event_types et 
 left join (
...
select 'Failed' as name, 0 as id, ifnull(fp.cnt,0)+ifnull(fo.cnt,0) as cnt 
 ...
 order by id");
	...
	public function getFailedServices() {
		$ret = [];
		$s = $this->ci->db->prepare("select (UNIX_TIMESTAMP()*1000-f.timestamp)/1000 as late_sec, h.id as host_id, s.id as serv_id, f.status, s.name as service, h.name as host, f.timestamp 
